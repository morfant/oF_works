...
void ofApp::update(){
    ...
    while(receiver.hasWaitingMessages()){
        ...
        if (m.getAddress() == "/from/max"){
            modeSet = m.getArgAsInt(0);
        } else if(m.getAddress() == "/from/sc"){
            modeSet = m.getArgAsInt(0);
        } else {
            cout << "Check OSC message!" << m << endl;
        }
    }
...
void ofApp::draw(){
    ...
    if (ofGetFrameNum() % 300 == 0 || ofGetFrameNum() == 0) {
        ...
        ofxOscMessage m;
        m.setAddress("/of/numLines");
        m.addIntArg(numOfLines);
        ...
        ofNoFill();
        ofSetColor(255);
        ofDrawBitmapString("color mode is " + to_string(modeSet), 100, 120); // msg
